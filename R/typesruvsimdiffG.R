...
for (i in 1:length(seeds)) {
  ...
  #fit <- lm(ruv4T1$W[pbc.metaD$cg_cov==ct,]~pbc.metaD$fk.tr[pbc.metaD$cg_cov==ct])
  #r2ruv4t1[[i]] <- r2(ruv4T1$W[pbc.metaD$cg_cov==ct,],pbc.metaD$fk.tr[pbc.metaD$cg_cov==ct])
  ...
  for (ct in celltypes) {
    ...
    fit2 <- lm(ruv4$W~pbc.metaD$fk.tr[pbc.metaD$cg_cov==ct]=='A')
    summary(fit2)
...
saveRDS(pvals,paste0(path,'/pvals_ruv_diffG.rds'))

saveRDS(sw2its,paste0(path,'/asw2_ruv_diffG.rds'))
saveRDS(swpits,paste0(path,'/aswp_ruv_diffG.rds'))
...
# 
# 
# saveRDS(plotsC, paste0(path,'/cobra_ruv_diffG.rds'))
# 
