...
pbpscounts <- sapply(names(cells_to_pbps),function(x)pbps(cells_to_pbps,Gr=x,ds1=ds2s))
...
for (i in 1:length(seeds)) {
  ...
  for(j in 1:length(celltypes)){
    ...
    sim.ct <- simulateDE(fullcount[!hk.ind, pbc.metaD$cg_cov==celltypes[j]], which_cols = samp_to_swap[pbc.metaD$cg_cov==celltypes[j]], prop_DE = 0.1)
    gene.D[, paste0('trueDE_',celltypes[j])] <- FALSE
    gene.D[!hk.ind,  paste0('trueDE_',celltypes[j])] <- sim.ct@elementMetadata@listData[["is_DE"]]
    sim.ct <- rbind(assays(sim.ct)$counts,fullcount[hk.ind, pbc.metaD$cg_cov==celltypes[j]])
    sim.ct <- sim.ct[gene.D$gene,]
    sim.ps[,pbc.metaD$cg_cov==celltypes[j] ] <- sim.ct
  }
  ...
  for (ct in celltypes) {
    ...
    colnames(design) <- c('(Intercept)','tr',paste0('W', 1:k), paste0('PCohort', 2:4))
    ...
    design <- model.matrix(~ sifull$fk.tr[sifull$sample_cell%in%orig.s]+ ruv3ps$W[orig.s,] ) 
    colnames(design) <-c('(Intercept)','tr',paste0('W', 1:k))
    
    
    pca <- calculatePCA(t(ruv3ps$newY[orig.s,]),ncomponents=10)
    aswPC <- ASW(pca,pbc.metaD$Processing_Cohort[pbc.metaD$cg_cov==ct])
    aswTr<- ASW(pca,pbc.metaD$fk.tr[pbc.metaD$cg_cov==ct])
    aswEt<- ASW(pca,pbc.metaD$pop_cov[pbc.metaD$cg_cov==ct])
...
saveRDS(truthits,paste0(path,'/truthpbps10_dgbal.rds'))
saveRDS(pvalits,paste0(path,'/pvalspbps10_dgbal.rds'))
saveRDS(aswits,paste0(path,'/aswpbps10_dgbal.rds'))

#i=2

# pvalsg <- column_to_rownames(pvalits[[i]]$T8,'ind') 
# padjsg<- column_to_rownames(padjits[[i]]$T8,'ind')
# truthg<- column_to_rownames(truthits[[i]]$T8,'ind')
# test <- COBRAData(pval=pvalsg,truth = truthg, padj = padjsg)
# #test <- COBRAData(pval=pvalsg,truth = truthg)
# #test <- calculate_adjp(test)
# perf <- calculate_performance(test,binary_truth = "trueDE")
# top <- prepare_data_for_plot(perf, colorscheme = "Dark2", facetted = TRUE)
# plot_fdrtprcurve(top,pointsize=0.5)
# plot_tpr(top)
# 
# 
# pvalits2 <- list_transpose(pvalits)
...
# plotscobratpr <- lapply(cobratoplot,plot_tpr)
# 
# plotsC <- list(plotscobra ,plotscobraC ,plotscobratpr )
# 
# #saveRDS(plotsC, paste0(path,'/cobra_limma_ruvps.rds'))
# saveRDS(plotsC, paste0(path,'/cobra_limma_ruvps_nonc.rds'))
# 
# 
# # pval$T4 <- column_to_rownames(pval$T4,'ind') 
# # truth$T4 <- column_to_rownames(truth$T4,'ind')
# # test <- COBRAData(pval=pval$T4,truth = truth$T4)
# # test <- calculate_adjp(test)
# # perf <- calculate_performance(test,binary_truth = "trueDE")
# # top <- prepare_data_for_plot(perf, colorscheme = "Dark2", facetted = TRUE)
# # plot_fdrtprcurve(top,pointsize=0.5)
