...
filter(PBPSC$samples,cg_cov=='T4')%>% group_by(Processing_Cohort,ind_cov,pop_cov,Age,Status,pbps) %>%
  ...
  ggplot(aes(y=ind_cov,x=Processing_Cohort, group=ind_cov, color= as.factor(count),shape=as.factor(pbps))) +
  geom_point(size=2) + geom_line() + scale_colour_brewer(palette='Set1') +
  labs(y = "Subject", x= "Processing cohort", color = "Replicates", shape='PBPS')+ theme_minimal()+
  ggtitle('Assays')
